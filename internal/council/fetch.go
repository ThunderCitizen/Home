...
func writeVotesJSON(meetings []Meeting) error {
	...
	for termLabel, termMeetings := range byTerm {
		// Pre-2022-11-15 meetings (the closed 2018-2022 term) map to an empty
		// term label. That term is frozen in the static votes_2018.json seed and
		// is no longer regenerated by the scraper, so skip it rather than index
		// termLabel[:4] on an empty string.
		if termLabel == "" {
			continue
		}

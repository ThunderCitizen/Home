...
- **Find Route is a checkbox-as-tab-stop** — `<input type="checkbox" id="trip-toggle">` lives next to its `<label>` inside the Features btn group, visually-hidden-but-focusable (sr-only style, no `display: none`). The label paints the btn; native click/keyboard toggles the checkbox; CSS uses `.route-panel:has(.trip-toggle:checked) ...` (not `~`) so the checkbox can sit in its semantic spot. No JS keyboard handler — let the browser do its thing.
